...
const loadHymn = async (number) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            let hymnText = (await to(axios.get(`https://raw.githubusercontent.com/GospelSounders/adventhymnals.old/master/tools/files/lyrics/CiS1908/${numberInText}.txt`)))[1].data
            loadedHymns[numberInText] = hymnText
        } catch (error) { }
        resolve()
    })
}
...
const workOnSingleFile = async (header) => {
    ...
    let firstLine = header["First Line"];
...
let missingNos = []
const main = async () => {
    let headers_ = fs.readFileSync("CiSHeader.txt", "utf-8").split(/\n/).filter(item => item.length > 0).map(item => JSON.parse(item))

    headers_.map(item => {
        headers[item.hymnNumber] = item
    });
    let numbers = [];
    let index = 0
    while (index++ <= 949) numbers.push(index)
    index = 0;
    // events.on("next", () => {
    //     if (++index > 949) {
    //         process.exit()
    ...
    server.on('message', async function (msg, info) {
        ...
        if ((index - 1) % interval == 0) {
            ...
            // let endAt = 
            while (newInd <= ((index + interval) <= 949 ? (index + interval) : 949)) arr.push(newInd++)
