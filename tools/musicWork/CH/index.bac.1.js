  ...
  for (let k in trackswithNotes_) {
    ...
    for (let j in trackswithNotes_[k]) {
      ...
      if (duration - trackswithNotes_[k][j].time !== 0) {
        // console.log(trackswithNotes_[k][j].time) // convert this time to quarterNotes
        // console.log(trackswithNotes_[k][j].quarterNotes)
        // console.log('has partials............................')
        partials = trackswithNotes_[k][j].time
        // console.log(j)
        // process.exit();  // check. start ---> partials....
...
let processTxtFile = async (path_) => {
  ...
  if (!hasRefrain) {
    ...
    try {
      ...
      console.log('-------------------------------------------->1546-3')
      let noteSyllableGroups_ = getSyllablesFromNotes(trackswithNotes_);
       ...
       * long and short... 
       */
      let lilypondFiles_ = createLilypondFiles(headerData, notesforAllVoices, stanzasHyphenated, timesignatureStr, bpm, songNumber, key, partials)
