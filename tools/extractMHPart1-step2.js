...
const save10sChapter = async (tens, tensChaptersPathsObj) => {
    ...
    let innerchapterTxt =
        `---
title: Millenial Harp - ${tmpe_}
metadata:
    description: |
        Millenial Harp - ${tmpe_}
    keywords:  |
        Millenial Harp, adventhymnals, advent hymnals, ${tmpe_}
...
`
    ...
    fs.writeFileSync(`millenial-harp/${tensChaptersPathsObj}/chapter.md`, innerchapterTxt)
}
...
const workOnSingleFile = async (header, index) => {
    try {
        ...
        let hymnText_ = fs.readFileSync(`MH/${index}`, 'utf-8')
        ...
        fs.ensureDirSync(`millenial-harp/${dirPath}`)
        let hymnText;
        ...
        let saveDirPath = `millenial-harp/${path_}.${saveTitle}`
        fs.ensureDirSync(saveDirPath)
        let urlPath = '/millenial-harp/' + saveDirPath.replace(/[0-9]+\./g, '')
        ...
        if (number === max) {
            fs.ensureDirSync("MHInfo")
            fs.writeFileSync(`MHInfo/titles.json`, JSON.stringify(titlesInfo))
            fs.writeFileSync(`MHInfo/poets.json`, JSON.stringify(poetsInfo))
            fs.writeFileSync(`MHInfo/topics.json`, JSON.stringify(topicsInfo)) // several
            fs.writeFileSync(`MHInfo/tunes.json`, JSON.stringify(tunesInfo))
            fs.writeFileSync(`MHInfo/composers.json`, JSON.stringify(composersInfo))
            fs.writeFileSync(`MHInfo/keys.json`, JSON.stringify(keysInfo))
            fs.writeFileSync(`MHInfo/years.json`, JSON.stringify(yearsInfo))
            fs.writeFileSync(`MHInfo/scriptures.json`, JSON.stringify(scripturesInfo))
            await save10sChapter(tensIndices, tensChaptersPathsObj)
            await save10sChapter(hundedsIndices, hundredsChaptersPathsObj)
...
`
            ...
            fs.ensureDirSync("millenial-harp/01.indices")
            fs.writeFileSync(`millenial-harp/01.indices/chapter.md`, indexContent)
        }
        let saveText =
            `---
title: |
    ${numberInText}. ${title} - Millenial Harp
metadata:
    description: |
        Millenial Harp ${numberInText}. ${title}. ${firstStanzaSingleLine}
    keywords:  |
        Millenial Harp, adventhymnals, advent hymnals, ${title}, ${firstLine}. ${firstLineRefrain ? firstLineRefrain : ''}
    author: Brian Onang'o
---
#### Advent Hymnals
