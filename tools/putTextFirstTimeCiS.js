...
let readTitle = async (path) => {
  ...
  try{
  let fileContents = fs.readFileSync(pathF.join("files", "lyrics", "created", "CiS", filePath), 'utf-8')
  ...
  fs.writeFileSync(pathF.join("files", "lyrics", "created", "CiS", filePath),fileContents)
  // console.log(fileContents)
  }catch(error) {
    console.log(error)
  }
